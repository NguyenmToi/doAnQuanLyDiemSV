...
{
    public partial class UserControlQLTKSV : UserControl
    {
        QL_DIEMSVEntities DB = new QL_DIEMSVEntities();
        QLTKSVsBUS _taiKhoans1SVBUS = new QLTKSVsBUS();    
        public UserControlQLTKSV()
        {
            InitializeComponent();
            DSTaiKhoan();
        }
        ...
        private void dgvTaiKhoanGV_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txtGhiChu.Text = dgvTaiKhoanSV.Rows[e.RowIndex].Cells[Constant.TKPQ_GhiChu].Value.ToString();
            }
        }
        ...
        private void UserControlQLTKSV_Load(object sender, EventArgs e)
        {
            cboTimKiem.SelectedIndex = 0;
        }
    }
